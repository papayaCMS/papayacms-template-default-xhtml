...
<xsl:template name="page-scripts-lazy">
  ...
  <xsl:call-template name="link-script">
    <xsl:with-param name="file">papaya/jquery.papayaFeedGallery.js</xsl:with-param>
  </xsl:call-template>
</xsl:template>
  ...
  <xsl:if test="$thumbs">
    <xsl:for-each select="$thumbs">
      <div class="galleryThumbnail">
        <xsl:variable name="thumbnailParameters">
        ...
        <a class="galleryThumbnailFrame"
           ...
           href="{a/@href}"
           title="{image/@title}"
           data-lightbox="{papaya-fn:json-encode($thumbnailParameters)}">
